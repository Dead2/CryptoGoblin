...
//#endif
    ...
    char buffer[64];
    snprintf(buffer, sizeof(buffer), GREEN("%s"), get_version_str_short().c_str());
    printer::inst()->print_str(buffer);
    printer::inst()->print_str(CYAN(" by ") GREEN("Dead2") CYAN(" CPU mining software under ") RED("GPLv3\n"));
    ...
    if((fDevDonationLevel*100) > 0.99){
        ...
        printer::inst()->print_str(RED("Dev donation is disabled. :(\n\n"));
    }
...
int do_benchmark(int block_version, int wait_sec, int work_sec)
{
    ...
    char jobid[64];
    memset(jobid,0,64);
    xmrstak::globalStates::inst().switch_work(xmrstak::miner_work(jobid, work, 128, 0, false, 0, 0), dat);
