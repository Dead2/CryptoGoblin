...
void disable_sigpipe()
{
    ...
    if (sigaction(SIGPIPE, &sa, 0) == -1)
        printer::inst()->print_msg(L1, RED("ERROR: Call to sigaction failed!"));
}
...
void executor::ex_main()
{
    ...
    if(pvThreads->size()==0)
    {
        printer::inst()->print_msg(L1, RED("ERROR: No miner backend enabled."));
        win_exit();
    }
    ...
    for(; i < pc; i++)
    {
        ...
        if(cfg.tls)
        {
            printer::inst()->print_msg(L1, RED("ERROR: Enabling TLS failed, miner was compiled without TLS support."));
            win_exit();
        }
#endif
    ...
    if(!xmrstak::params::inst().poolURL.empty() && !already_have_cli_pool)
    {
        ...
        {
            printer::inst()->print_msg(L1, RED("ERROR: You didn't specify the username / wallet address for %s"), xmrstak::params::inst().poolURL.c_str());
            win_exit();
        }
...
void executor::hashrate_report(std::string& out)
{
    ...
    for( uint32_t b = 0; b < 4u; ++b)
    {
        ...
        if(nthd != 0)
        {
            ...
            out.append(CYAN("| ") YELLOW("ID ") CYAN("|    ") YELLOW("10s ") CYAN("|    ") YELLOW("60s ") CYAN("|    ") YELLOW("15m ") CYAN("|"));
            if(nthd != 1)
                out.append(CYAN(" ") YELLOW("ID ") CYAN("|    ") YELLOW("10s ") CYAN("|    ") YELLOW("60s ") CYAN("|    ") YELLOW("15m ") CYAN("|\n"));
            ...
            out.append(CYAN("Totals (")).append(name).append(CYAN("): "));
            out.append(hps_format_color(fTotalCur[0], num, sizeof(num)));
            out.append(hps_format_color(fTotalCur[1], num, sizeof(num)));
            out.append(hps_format_color(fTotalCur[2], num, sizeof(num)));
            out.append(CYAN(" H/s\n"));
