...
TARGETS("sse4.2,sse4a,sse3,default")
...
ALIGN(64) FLATTEN2 void soft_cn_implode_scratchpad(const __m128i* input, __m128i* output)
{
    ...
    if(PREFETCH){
        _mm_prefetch((const char*)input + 0, _MM_HINT_T1);
        _mm_prefetch((const char*)input + 4, _MM_HINT_T1);
    }
    ...
    {
        if(PREFETCH)
            _mm_prefetch((const char*)input + i + 8, _MM_HINT_T1);
        ...
        if(PREFETCH)
            _mm_prefetch((const char*)input + i + 12, _MM_HINT_T1);
