...
ALIGN(64) void cryptonight_hash(const void* input, size_t len, void* output, cryptonight_ctx* ctx){
    ...
    for(size_t i = 0; i < ITERATIONS; i++)
    {
        ...
        if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_ipbc || ALGO == cryptonight_stellite)
            if(SOFT_AES)
                soft_cryptonight_monero_tweak<ALGO>((uint64_t*)&l0[idx0 & MASK], _mm_xor_si128(bx0, cx));
            else
        ...
        if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_stellite)
            l0_p[1] = ah0 ^ monero_const;
        else if (ALGO == cryptonight_ipbc)
            l0_p[1] = ah0 ^ monero_const ^ l0_p[0];
        else
            l0_p[1] = ah0;
...
ALIGN(64) void cryptonight_double_hash(const void* input, size_t len, void* output, cryptonight_ctx** ctx){
    ...
    for (size_t i = 0; i < ITERATIONS; i++)
    {
        ...
        if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_ipbc || ALGO == cryptonight_stellite)
            if(SOFT_AES)
                soft_cryptonight_monero_tweak<ALGO>((uint64_t*)&l1[idx1 & MASK], _mm_xor_si128(bx1, cx));
            else
        ...
        if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_stellite)
            ...
            ((uint64_t*)&l0[idx0 & MASK])[1] = axh0 ^ monero_const[0] ^ ((uint64_t*)&l0[idx0 & MASK])[0];
        else
        ...
        if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_stellite)
            ...
            ((uint64_t*)&l1[idx1 & MASK])[1] = axh1 ^ monero_const[1] ^ ((uint64_t*)&l1[idx1 & MASK])[0];
        else
...
#define CN_STEP2(a, b, c, l, ptr, idx)                          \
        ...
        if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_ipbc || ALGO == cryptonight_stellite) \
            if(SOFT_AES) \
                soft_cryptonight_monero_tweak<ALGO>((uint64_t*)ptr, b); \
            else \
