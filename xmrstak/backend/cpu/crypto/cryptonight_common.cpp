...
size_t cryptonight_init(size_t use_fast_mem, size_t use_mlock, alloc_msg* msg)
{
    ...
    {
        printer::inst()->print_msg(L0, YELLOW("Attempting to elevate to admin because we need to grant fast memory privileges."));
        RequestElevation();

        if(AddLargePageRights())
        {
            msg->warning = GREEN("Added SeLockMemoryPrivilege to the current account. You need to reboot for it to work");
            bRebootDesirable = TRUE;
        }
        else
            msg->warning = RED("Obtaining SeLockMemoryPrivilege failed.");
...
cryptonight_ctx* cryptonight_alloc_ctx(size_t use_fast_mem, size_t use_mlock, alloc_msg* msg)
{
    ...
    if(use_fast_mem == 0)
    {
        ...
        ptr->long_state = (uint8_t*)_mm_malloc(hashMemSize, 2*1024*1024);
        ptr->ctx_info[0] = 0;
        ptr->ctx_info[1] = 0;
        return ptr;
    }
