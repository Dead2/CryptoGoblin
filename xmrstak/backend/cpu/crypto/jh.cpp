/*
Based on original SSE2 optimized JH hash, further simplified and optimized for CryptoNight by Hans Kristian Rosbach aka Dead2.
*/
...
void xmr_jh256(const BitSequence *data, BitSequence *hashval){
    ...
    memcpy(state.buffer, data+192, (((XMR_DATABITLEN - 2048) & 0x1ff) >> 3)+1);


    // ## Final ##
    uint8_t start = ((XMR_DATABITLEN & 0x1ff) >> 3);
    memset((void *)&state.buffer[start], 0, (64 - start));

    //pad and process the partial block when databitlen is not multiple of 512 bits, then hash the padded blocks
    state.buffer[((XMR_DATABITLEN & 0x1ff) >> 3)] |= 1 << (7- (XMR_DATABITLEN & 7));
    F8(&state);
    memset(state.buffer,0,64);
    state.buffer[63] = XMR_DATABITLEN & 0xff;
    state.buffer[62] = (XMR_DATABITLEN >> 8) & 0xff;
    state.buffer[61] = (XMR_DATABITLEN >> 16) & 0xff;
    state.buffer[60] = (XMR_DATABITLEN >> 24) & 0xff;
    state.buffer[59] = (XMR_DATABITLEN >> 32) & 0xff;
    state.buffer[58] = (XMR_DATABITLEN >> 40) & 0xff;
    state.buffer[57] = (XMR_DATABITLEN >> 48) & 0xff;
    state.buffer[56] = (XMR_DATABITLEN >> 56) & 0xff;
    F8(&state);

    //truncting the final hash value to generate the message digest
    STORE(state.x6,hashval);
    STORE(state.x7,hashval+16);
}
