	...
	void printConfig()
	{
		...
		if(!detectL3Size() || L3KB_size < 1024 || L3KB_size > 102400)
		{
			if(L3KB_size < 1024 || L3KB_size > 102400)
				printer::inst()->print_msg(L0, RED("Autoconf failed: L3 size sanity check failed - %u KB."), L3KB_size);

            printer::inst()->print_msg(L0, RED("Autoconf FAILED!"));
		...
		printer::inst()->print_msg(L0, GREEN("Autoconf L3 size detected at %u KB."), L3KB_size);

		detectCPUConf();

		printer::inst()->print_msg(L0, GREEN("Autoconf core count detected as %u on %s."), corecnt,
...
private:
	bool detectL3Size()
	{
		...
		if(strcmp(cpustr, "GenuineIntel") == 0)
		{
			...
			{
				printer::inst()->print_msg(L0, RED("Autoconf failed: Couln't find L3 cache page."));
				return false;
			}
		...
		{
			printer::inst()->print_msg(L0, RED("Autoconf failed: Unknown CPU type: %s."), cpustr);
			return false;
		}
	}
