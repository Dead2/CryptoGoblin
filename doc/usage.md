...
## Usage on Windows
1) Double click the `CryptoGoblin.exe` file
...
2) Start the miner with `./CryptoGoblin`

## Command Line Options

The miner allow to overwrite some of the settings via command line options.
Run `CryptoGoblin --help` to show all available command line options.
