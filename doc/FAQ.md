...
If you set up the user rights properly ([see above](https://github.com/Dead2/CryptoGoblin/blob/master/doc/FAQ.md#selockmemoryprivilege-failed)), and your system has 4-8GB of RAM (50%+ use), there is a significant chance that there simply won't be a large enough chunk of contiguous memory because Windows is fairly bad at mitigating memory fragmentation.
...
This means that the program attempted to use an instruction that the cpu does not support. This might be caused by choosing to compile for native instead of generic architecture.
...
If your antivirus software flags **CryptoGoblin**, it will likely move it to its quarantine area. You may have to whitelist **CryptoGoblin** in your antivirus.
...
If the miner is compiled for Monero and Aeon than you can change
 ...
 - run `CryptoGoblin --help` to see all supported currencies and algorithms
...
If your coin you want to mine is not listed please check the documentation of the coin and try to find out if `cryptonight`, `cryptonight-lite` or `cryptonight-heavy` is the used algorithm.
