...
There are two easy ways to set variables for `cmake` to configure *CryptoGoblin*
- use the ncurses GUI
  - `ccmake ..`
  - edit your options
...
- `CMAKE_INSTALL_PREFIX` install miner to the home folder
  - `cmake .. -DCMAKE_INSTALL_PREFIX=$HOME/CryptoGoblin`
  - you can find the binary and the `config.txt` file after `make install` in `$HOME/CryptoGoblin/bin`
